...
of the synoptic gospels for training and evaluating transformer-based models on
Koine Greek and for studying editorial tendencies and source relationships.
...
- The Needleman-Wunsch alignment uses lemma+POS matching with fixed gap
  ...
  alignments.
- **The direction of literary copying (who copied whom) is not identifiable
  from these texts alone** — a negative result documented in the parent project
  ([`DIRECTION_NEGATIVE_RESULT.md`](https://github.com/abderahmane-ai/SynoptiQ/blob/main/docs/DIRECTION_NEGATIVE_RESULT.md)).
  Use the corpus for representation learning and analysis of dependence *beyond*
  Mark, not for inferring copying direction.
...
- [KoineFormer](https://huggingface.co/ainouche-abderahmane/koineformer) —
  LoRA-adapted GreTa encoder trained on Koine Greek, evaluated on this corpus.
- [Koine-T5](https://huggingface.co/ainouche-abderahmane/koine-t5) — multitask
  seq2seq model (POS · lemma · infilling · synoptic transfer); try the

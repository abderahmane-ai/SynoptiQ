...
\item \textbf{KoineFormer}---a domain-adapted Koine Greek T5 encoder-decoder that reaches 96.5\% POS accuracy, produced by training only 3.7M of 220M parameters.\footnote{\url{https://huggingface.co/ainouche-abderahmane/koineformer}}

\item \textbf{Evidence that parameter-efficient adaptation outperforms full fine-tuning for small historical corpora}---full fine-tuning of all 220M parameters achieves \emph{worse} downstream accuracy (96.1\%), because overfitting dominates when the corpus is smaller than a single novel.
\end{enumerate}
...
\quad--- Attention layers ($W_{q}, W_{v}, W_{o}$, self + cross) & 2.65M & 2.65M & --- \\
\quad--- Feed-forward layers ($W_{o}$ output projection) & 1.08M & 1.08M & --- \\
\midrule
...
The domain-adaptive pre-training (DAPT) corpus consists of approximately 1.5 million tokens of Koine Greek text, drawn from the following sources:
\begin{itemize}
    \item \textbf{Koine Greek texts (70\%)}: The SBLGNT \citep{holmes2010} (27 books, $\sim$773K tokens) and the Apostolic Fathers ($\sim$732K tokens), including works such as 1--2 Clement, the letters of Ignatius, and the Didache.
...
\toprule
\tblhead\textbf{Model} & \textbf{POS} & \textbf{POS Err.} & \textbf{Lemma} & \textbf{Params} & \textbf{Checkpoint} \\
\midrule
GreTa (zero-shot) & 95.32\% & 4.68\% & 82.37\% & 0 & 880 MB \\
Full fine-tuning & 96.11\% & 3.89\% & --- & 220M & 880 MB \\
\rowcolor{terracotta!12}\textbf{KoineFormer (LoRA)} & \textbf{96.62\%} & \textbf{3.38\%} & 81.34\% & \textbf{3.7M} & \textbf{14 MB} \\
\bottomrule
\end{tabular}
\caption{\textbf{Two-task evaluation.} KoineFormer improves POS accuracy by 1.30 points over zero-shot (28\% relative error reduction) while training 60$\times$ fewer parameters than full fine-tuning. Lemmatisation accuracy is comparable between zero-shot and DAPT (82.4\% vs.\ 81.3\%)---span-corruption DAPT with a classical replay buffer does not improve vocabulary-level tasks. Full fine-tune lemma results are pending.}
\label{tab:results}
\end{table}

The results demonstrate four key findings:
\begin{enumerate}
    \item \textbf{Strong zero-shot POS transfer}: GreTa, trained exclusively on Classical Greek, achieves 95.32\% POS accuracy on Koine---the domain gap is smaller than previously assumed.

    \item \textbf{Effective and task-selective adaptation}: KoineFormer improves POS accuracy by 1.30 points (28\% error reduction) with only 3.7M trainable parameters. Lemmatisation accuracy is unchanged (82.4\% vs.\ 81.3\%)---span-corruption DAPT improves syntactic representations but does not expand vocabulary coverage, since the corpus is small and the classical replay buffer reintroduces Attic forms.

    \item \textbf{Overfitting in full fine-tuning}: Training all 220M parameters yields only 96.11\% POS accuracy, with unstable training dynamics (Table~\ref{tab:loss}).

    \item \textbf{Task selectivity as a diagnostic}: The divergence between POS improvement and lemma stagnation reveals what the Classical$\rightarrow$Koine domain gap actually consists of---primarily syntactic register differences rather than vocabulary gaps. This is methodologically useful for future domain adaptation work in historical languages.
\end{enumerate}
...
To complement the quantitative evaluation, we test each model's ability to
fill a missing word in a verse.  A comma replaces the target word in the
input, and the model generates a continuation.  Table~\ref{tab:qual} reports
the actual Greek output from each model.

\begin{table}[H]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{1.5cm}L{2.6cm}L{4.25cm}L{4.25cm}}
\toprule
\tblhead\textbf{Verse} & \textbf{Input (masked)} & \textbf{Zero-Shot GreTa} & \textbf{KoineFormer (DAPT)} \\
\midrule
\textbf{Mark 1:1}
& \gk{Ἀρχὴ τοῦ, Ἰησοῦ Χριστοῦ υἱοῦ θεοῦ.}
\newline Target: \gk{εὐαγγελίου} \textit{(gospel)}
& \gk{θεοῦ.}
\newline\textit{Collapse to one word}
& \gk{πρώτην τοῦ εὐαγγελίου ἰησοῦ χριστοῦ υἱοῦ θεοῦ.}
\newline\textit{Restores gospel + correct prefix} \\
\midrule
\rowcolor{papyrus}
\textbf{Luke 1:47}
& \gk{...ἐπὶ τῷ θεῷ τῷ, μου}
\newline Target: \gk{σωτῆρί} \textit{(savior)}
& \gk{, καὶ ἠγαλλίασεν... τῷ σωτῆρί μου.}
\newline\textit{Gets word; retains comma noise}
& \gk{τὸν κύριον, καὶ ἠγαλλίασεν... τῷ σωτῆρί μου.}
\newline\textit{Clean, fluent clause} \\
\midrule
\textbf{Gen 1:1}
& \gk{Ἐν ἀρχῇ, ὁ θεὸς τὸν οὐρανὸν...}
\newline Target: \gk{ἐποίησεν} \textit{(created)}
& \gk{ἐν ἀρχῇ ἐποίησεν ὁ θεὸς...}
\newline\textit{Perfect (\gk{ἐποίησεν})}
& \gk{ἐν ἀρχῇ ἦν ὁ θεὸς...}
\newline\textit{John 1:1 override (\gk{ἦν} for \gk{ἐποίησεν})} \\
\midrule
\textbf{Gal 5:22}
& \gk{...εἰρήνη, χρηστότης ἀγαθωσύνη}
\newline Target: \gk{μακροθυμία} \textit{(patience)}
& \gk{πίστις πραότης, ἐγκράτεια.}
\newline\textit{Skips masked slot}
& \gk{ἀγαθωσύνη ἀγαθωσύνη χρηστότης...}
\newline\textit{Loops on nearby words} \\
\bottomrule
\end{tabular}
\caption{\textbf{Qualitative fill-in-the-blank evaluation.}  A comma replaces the target word in the input; each model generates a continuation.  In Mark~1:1, the zero-shot model collapses to a single token (\gk{θεοῦ}), while KoineFormer correctly restores \gk{εὐαγγελίου} (gospel) with a grammatical modifier---a direct demonstration of domain adaptation correcting a baseline failure.  In Luke~1:47, both models recover \gk{σωτῆρί} (savior), but KoineFormer produces a clean clause while the zero-shot model retains input noise.  Genesis~1:1 reveals an interesting interference effect: DAPT reinforcement of the Johannine prologue causes KoineFormer to substitute \gk{ἦν} (was) for \gk{ἐποίησεν} (created)---a theological prior overwriting the Septuagintal reading.  Both models struggle with the list-structured Galatians~5:22 context, a known limitation of span-corruption training.}
\label{tab:qual}
\end{table}

These examples illustrate three patterns.  First, KoineFormer's domain
adaptation enables recovery where the zero-shot model fails entirely
(Mark~1:1).  Second, even when both models succeed, the adapted model
produces cleaner, more grammatical output (Luke~1:47).  Third,
domain-adaptive pre-training can introduce interpretive bias---the
Genesis~1:1 case shows the model's training distribution favouring a
distinctly Christian textual memory over a Septuagintal one.  This
honest failure mode is methodologically important: it demonstrates
that DAPT alters the model's generative priors, which downstream
...
\begin{itemize}
    \item \textbf{Task breadth}: The current evaluation covers POS tagging and lemmatisation via linear probes. Future work will extend to dependency parsing, semantic role labeling, and other syntactic tasks that require deeper linguistic representations.

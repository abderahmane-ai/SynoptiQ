...
\toprule
\tblhead\textbf{Model} & \textbf{POS} & \textbf{POS Err.} & \textbf{Lemma} & \textbf{Params} & \textbf{Checkpoint} & \textbf{GPU Cost} \\
\midrule
GreTa (zero-shot) & 95.32\% & 4.68\% & 82.37\% & 0 & 880 MB & \$0.00 \\
Full fine-tuning & 96.11\% & 3.89\% & --- & 220M & 880 MB & \textasciitilde\$23 \\
\rowcolor{terracotta!12}\textbf{KoineFormer (LoRA)} & \textbf{96.62\%} & \textbf{3.38\%} & 81.34\% & \textbf{3.7M} & \textbf{14 MB} & \textbf{\$0.35} \\
\bottomrule
\end{tabular}
\caption{\textbf{Two-task evaluation.} KoineFormer improves POS accuracy by 1.30 points over zero-shot (31\% relative error reduction) while training 60$\times$ fewer parameters and costing 66$\times$ less than full fine-tuning. Lemmatisation accuracy is comparable between zero-shot and DAPT (82.4\% vs.\ 81.3\%)---span-corruption DAPT with a classical replay buffer does not improve vocabulary-level tasks. Full fine-tune lemma results are pending.}
\label{tab:results}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Overfitting in full fine-tuning}: Training all 220M parameters yields only 96.11\% POS accuracy at significantly higher cost, with unstable training dynamics (Table~\ref{tab:loss}).

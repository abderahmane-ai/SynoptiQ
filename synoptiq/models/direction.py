...
_MDL_SWAP_PAIRS = ((0, 1), (3, 4), (6, 7))
_MDL_NEGATE = (2, 5, 8, 9, 10)
_MDL_N_FEATURES = 11
_MDL_N_SIGNED = 8          # 3 pair-differences + 5 negating features
_MDL_N_INDEP = 3 + _MDL_N_SIGNED  # 3 symmetric means + |signed|


class MDLDirectionHead(nn.Module):
    """Swap-equivariant linear head over the 11 NLL codelength features.

    Emits logits [d, -d, i] (A_to_B, B_to_A, independent), so swapping A and B exchanges
    the directed classes by construction. Trained only on the synthetic same-author,
    length-decorrelated redaction corpus — never on synoptic 2SH labels — so it cannot
    exploit authorship or length, and Phase 6's model comparison stays non-circular.
    """

    def __init__(self) -> None:
        super().__init__()
        self.direction_head = nn.Linear(_MDL_N_SIGNED, 1, bias=False)
        self.independence_head = nn.Linear(_MDL_N_INDEP, 1, bias=True)
        self.register_buffer("feature_mean", torch.zeros(_MDL_N_FEATURES))
        self.register_buffer("feature_std", torch.ones(_MDL_N_FEATURES))

    @torch.no_grad()
    def set_feature_stats(
        self, mean: torch.Tensor, std: torch.Tensor, min_std: float = 1e-6,
    ) -> None:
        """Fix standardization stats; swap pairs share stats, negators center at 0.
